import numpy as np
import torch
from PupilSense.detectron2.detectron2.config import get_cfg
import cv2
...
class Inference:
    ...
    def predict_batch(self, save_images=True) -> float:
        """
        ...
        Returns:
            The average pupil to average iris ratio or if both are nonexistent
        """
            ...
            if len(bbox_l) != 0:
                ...
                for index, score in instances_with_scores:
                    ...
                    if iris_flag and pupil_flag:
                        break
        # Antonio's Edits - ts sloppily coded...
        if total_images == 0:
            return -1
        if info["radiusPupil"]:
            valid_pupil_radius = np.mean(info["radiusPupil"])
        else:
            valid_pupil_radius = 0
        if info["radiusIris"]:
            valid_iris_radius = np.mean(info["radiusIris"])
        else:
            valid_iris_radius = 0
        if valid_iris_radius == 0 or valid_pupil_radius == 0:
            print("Average valid pupil to iris ratio:", 0)
            return 0
        print(f"Total Images = {total_images}, Valid Images = {valid_images}, "
              "Total Instances = {total_instances}, Iris Instances = {iris_instances}, "
              "Pupil Instances = {pupil_instances}")
        try:
            ratio = valid_pupil_radius / valid_iris_radius
        except ZeroDivisionError:
            print("Div0 error")
            ratio = 0
        finally:
            print("Average valid pupil to iris ratio:", ratio)
        return ratio
